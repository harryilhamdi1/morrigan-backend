...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        // KPI Scores
        const { data: kpiScores, error: kErr } = await supabase.from('kpi_scores').select('*');
        if (kErr) throw kErr;

        // Action Plans (To use for National Priorities / VoC fallback)
        const { data: actionPlans, error: aErr } = await supabase.from('action_plans').select('*, stores(*)');
        if (aErr) throw aErr;
        ...
        const buildAgg = (levelObj, record, wave, regionOrBranch) => {
            ...
            Object.entries(record.sections).forEach(([secName, val]) => {
                ...
                if (val < 86) levelObj[wave].sections[secName].critical++;
            });
        };
