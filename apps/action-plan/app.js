...
async function initActionPlanDashboard() {
    ...
    try {
        ...
        // Fetch Stores
        const { data: storesData, error: sErr } = await supabase.from('stores').select('*');
        if (sErr) throw sErr;

        // Fetch Action Plans with Approvals
        const { data: apDbData, error: apErr } = await supabase.from('action_plans').select('*, stores(*), approvals(*)');
...
async function updateApproval(taskId, level, action) {
    ...
    try {
        ...
        const { error: appErr } = await supabase.from('approvals')
        ...
        const { error: statErr } = await supabase.from('action_plans')
